...
import { AuthGrpcExceptionCode } from './exception-client-code.constant';
...
import { generateAuthException } from './service-exception-generator';
...
export class AllExceptionsFilter implements RpcExceptionFilter {
  private static logger: Logger = new Logger(AllExceptionsFilter.name);
    ...
    return throwError(() =>
      new UnavailableRpcException(AuthGrpcExceptionCode.GOT_ISSUE).getError(),
    );
  }
}
